...
export default function BeritaAcaraPengembalianForm({ onCreated, initialSn }) {
  ...
  useEffect(() => {
    ...
    if (match) {
      ...
      setForm(f => ({
        ...
        spek_processor:       f.spek_processor || match.cpu || '',
        spek_ram:             f.spek_ram       || (match.ram_gb ? `${match.ram_gb} GB` : ''),
        spek_storage:         f.spek_storage   || match.storage_summary || (match.storage_gb ? `${match.storage_gb} GB` : ''),
  ...
  function handleSnSelect(laptop) {
    ...
    setForm(f => ({
      ...
      spek_processor:       f.spek_processor || laptop.cpu || '',
      spek_ram:             f.spek_ram       || (laptop.ram_gb ? `${laptop.ram_gb} GB` : ''),
      spek_storage:         f.spek_storage   || laptop.storage_summary || (laptop.storage_gb ? `${laptop.storage_gb} GB` : ''),
      pengembalian_nama:    borrower?.nama    ?? f.pengembalian_nama,
      pengembalian_jabatan: borrower?.jabatan ?? f.pengembalian_jabatan,
    }))
    setError(null)
